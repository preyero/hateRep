...
def krippendorf(df: pd.DataFrame, rater_col: str, subject_col: str, rating_col: str):

    print(f'computing Krippendorf on {rating_col}')
    # Transform to data format, e.g.,
    # rating_table = [[np.nan, np.nan, np.nan, np.nan, np.nan, 3, 4, 1, 2, 1, 1, 3, 3, np.nan, 3], # User 1
    #                 [1, np.nan, 2, 1, 3, 3, 4, 3, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan], # User 2
    #                 [np.nan, np.nan, 2, 1, 3, 4, 4, np.nan, 2, 1, 1, 3, 3, np.nan, 4]] # User 3
...
def get_alpha_and_deltas(data_subset: pd.DataFrame, label: str):
    ...
    return [round(alpha_1, 3), round(alpha_2, 3), round(alpha_2-alpha_1, 3)]